...
class PersonsController
{
    ...
    public function controlsRules(array $personPost, string $personItem, Person $editedPerson = null): array
    {
        $errors = [];
        ...
        if ($personItem === 'agent' && !isset($personPost['personSpecialities'])) {
            $errors['blank_personSpecialities'] = '<li class="error">La <b>SPÉCIALITÉ</b> est obligatoire sur la fiche ' . ($personItem === 'target' ? 'Cible' : ucfirst($personItem)) . '</li>';
        }
        ...
        // Vérifie que, si on change le pays d'une personne, celui ci n'est pas affecté à une mission, sinon message d'erreur selon si les règles suivantes ne sont respectées
         ...
         if ($editedPerson !== null) {
             if ($personPost['nationalityPerson'] !== $editedPerson->getNationality()) {
                 if (!empty($editedPerson->getMissions())) {
                     foreach ($editedPerson->getMissions() as $mission) {
                         if ($personItem === 'contact') {
                             if ($personPost['nationalityPerson'] !== $mission->getCountry()) {
                                 $errors['nationalityChange_' . $editedPerson->getId() . '-' . $mission->getId_mission()] = 
