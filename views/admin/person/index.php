...
$title = 'Spy Agency - Agents - Admin';
$styleFolder = '../../../styles/';
$styleSubFolder = 'admin/agent/';
...
if ($personItem === 'agent') {
    $agentsSpecialitiesController->hydrateAgents($personsListFiltered, $specialitiesList);
}
            ...
            <div class="filtersItemAndTitle">
                ...
                <div class="filtersItem">
                    <div class="personsFiltersLine">
                        <div class="labelAndFilter">
                            ...
                            <select name="personFilter[]" id="personFilter" multiple class="filter">
                                ...
                                <?php foreach($personsList as $person) : ?>
                                    <option 
                                        value="<?= $person->getId() ?>"
                                        <?php if (isset($_GET['personFilter'])): ?>
                                            <?php if (in_array($person->getId(), $_GET['personFilter'])): ?>
                                                selected
                                            <?php endif ?>
                                        <?php endif ?>
                        ...
                        <?php if ($personItem === 'agent'): ?>
                            <div class='labelAndFilter'>
                                <label for="specialitiesPersonfilter" class="filterTitle">Spécialité</label>
                                <select name="specialitiesPersonFilter[]" id="specialitiesPersonFilter" multiple class="filter">
                                    ...
                                    <?php foreach($specialitiesList as $speciality) : ?>
                                        <option
                                            ...
                                            <?php if (isset($_GET['specialitiesPersonFilter'])): ?>
                                                <?php if (in_array($speciality->getId_speciality(), $_GET['specialitiesPersonFilter'])): ?>
                                                    selected
                                                <?php endif ?>
                                            <?php endif ?>
                    ...
                    <div class="personsFiltersLine">
                        <div class='labelAndFilter'>
                            <label for="nationalityPersonFilter" class="filterTitle">Nationalité</label>
                            <select name="nationalityPersonFilter" id="nationalityPersonFilter" class="filter nationalityPersonSelect">
                                <option value="" class="headerSelect">Sélectionnez une nationalité</option>
                                <?php foreach($countriesList as $country) : ?>
                                    <option
                                        value="<?= $country['country'] ?>"
                                        <?php if (isset($_GET['nationalityPersonFilter']) && strlen($_GET['nationalityPersonFilter']) > 0): ?>
                                            <?php if ($country['country'] === $_GET['nationalityPersonFilter']): ?>
                                                selected
                                            <?php endif ?>
                                        <?php endif ?>
                        ...
                        <div class='labelAndFilter'>
                            <label for="startBirthdatePersonFilter" class="filterTitle">Date de naissance de début</label>
                            <input
                                type="date"
                                id="startBirthdatePersonFilter"
                                name="startBirthdatePersonFilter"
                                class="filter"
                                <?php if (isset($_GET['startBirthdatePersonFilter'])): ?>              
                                    value=<?= $_GET['startBirthdatePersonFilter'] ?>
                                <?php endif ?>
                            >
                        </div>
                        <div class='labelAndFilter'>
                            <label for="endBirthdatePersonFilter" class="filterTitle">Date de naissance de fin</label>
                            <input
                                type="date"
                                id="endBirthdatePersonFilter"
                                name="endBirthdatePersonFilter"
                                class="filter"
                                <?php if (isset($_GET['endBirthdatePersonFilter'])): ?>              
                                    value=<?= $_GET['endBirthdatePersonFilter'] ?>
                                <?php endif ?>
                            >
                        </div>
                    </div>
    ...
    <?php foreach($personsListFiltered as $person): ?>
        <li class="person">
            <div class="headerPerson">
                <p class="personItem"><?= ucfirst($personItem) . ' ' . $person->getId() ?></p>
            </div>
            <div class="infosPerson">
                ...
                <?php if($isAdmin) : ?>
                    <div class="actionBtns">
                        <a href="<?= $personItem . '/' . $person->getId() .'/edit'?>" class="editBtn actionBtn">
                            <span>
