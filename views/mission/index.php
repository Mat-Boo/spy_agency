...
<?php if (isset($_GET['delete'])): ?>
    <p class="deleteConfirmMessage"> <?= 'La mission ' . $_GET['delete'] . ' a bien été supprimée.' ?></p>
<?php endif ?>
                                ...
                                <?php foreach($missionsController->getStatus() as $status) : ?>
                                    <div style="background:<?= $status['background'] ?>">
                                        <input
                                            type="checkbox"
                                            id="<?= $status['status'] ?>"
                                            name=<?= "statusMissionFilter[]" ?>
                                            value="<?= $status['status'] ?>"
                                            class="filter"
                                            <?php if (isset($_GET['statusMissionFilter'])): ?>
                                                <?php if (in_array($status['status'], $_GET['statusMissionFilter'])): ?>
                                                    checked
                                                <?php endif ?>
                                            <?php endif ?>
                                        >
                ...
                <div class="detailsInfos">
                    ...
                    <?php foreach(['agent', 'contact', 'target'] as $person): ?>
                        <div class="infosItem <?= $person . 's' ?>">
                            ...
                            <ul>
                            <?php foreach($mission->{'get' . ucfirst($person . 's')}() as ${$person}): ?>
                                <li>
